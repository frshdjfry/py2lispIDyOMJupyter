...
SingleViewpoint = Literal[
    ...
    'registral-direction', 'intervallic-difference', 'registral-return', 'proximity', 'closure',
     # Derived viewpoints from derived.lisp
    'h-cpitch', 'h-cpitch-class', 'pi-chord', 'pi-chord-type', 'pc-chord', 'pc-chord-type', 'pc-chord-rel-prev-bass',
    'pc-set', 'pc-set-type', 'pc-set-rel-bass', 'pc-set-rel-prev-bass', 'pc-set-rel-root', 'bass-pc',
    'bass-int', 'bass-pc-rel-root', 'root-pc', 'root-int', 'sd-set', 'bass-sd', 'root-sd', 'pi-chord-identity',
    'pc-chord-identity', 'pc-set-identity', 'vintcc', 'csdc', 'gct-root', 'gct-chord-type', 'gct'
]
...
class RequiredParameters(Parameters):
    ...
    texture: str = None  # Add this line
    ...
    def to_lisp_command(self) -> str:
        ...
        if self.texture:  # Check if texture is specified and append to the command
            command += f' :texture {self.texture}'
        return command
...
class OutputParameters(Parameters):
    ...
    # the default output path in py2lispModified is experiment_history/THIS_EXP/experiment_output_data_folder/
